...
Understanding the Gross Domestic Product (GDP) production function is a cornerstone of macroeconomic analysis, it allows us to understand how economies grow and develop, in turn allowing for optimal policies and business strategy [@barro1995sala; @solow1957technical; @mankiw1992contribution]. The primary aim of this paper will be to compare the results of several classical econometric techniques, as well as more modern machine learning approaches. For this purpose the paper will analyse the impact of capital stock, human capital and population on GDP. The seminal work of @solow1957technical on the Solow growth model show that the level of physical capital is an important determinant of GDP, while the augmentation introduced by @mankiw1992contribution show that it is important to also include human capital to understand GDP. We also include population based on the work of @acemoglu2007disease that show that a larger workforce is positively correlated with economic growth. This analysis is conducted across numerous countries and near on 70 years of data so as to also look at differences in the production function between countries and years.

This paper will look at several approaches to finding a model that can represent the GDP production function as accurately as possible. A traditional mothod that has been used in econometrics is an Ordinary Least Squares (OLS) model which allows for easy implementaion and interpretation, however the assumption of linear relationships; weakness to outliers and reliance on the independence assumption can lead to biased estimates. Two-Way Fixed Effects (TWFE) models can use panel datasets, controlling for country and year indivudual effects, to try and limit unobservable heterogeneity. This can lead to more robust estimation in panel datasets than individual OLS models. However, this method relies on homogenous effects of the independent variables across countries and years and the parallel trends assumption. An Autoregressive Integrated Moving Average (ARIMA) model allows for a production function that uses the past values of GDP to estimate future values, this gives a better understanding of the relationship of the past values of GDP on current GDP. The shortcomings of the ARIMA model lie in the inability to account for external factors and a similar linear assumption to the OLS model. A Gradient Boosting Machine (GBM) will be used as a machine learning approach to estimating the GDP production function, this approach allows for mapping complex non-linear relationships between variables of interest and increasing the predictive power of the function by doing so. The shortcoming of the GBM model comes from the "black-box" nature of the model, this makes it difficult to interpret the findings of the model. This research project will aim to provide a holistic analysis of the GDP production function by leveraging the strengths of the above models in a ensemble modelling approach, which takes an optimised combination of the above models to approximate the production function. These insights can provide a wider understanding of the relationships between the variables of interest, and can add to the literature on the validity of a combined classical econometrics and modern machine learning approach in economic analysis.

# Literature Review {-}

Lit Review Intro

Paul Samuelson, a winner of the Nobel Memorial Prize in Economics, described econometrics in the following way, "Econometrics may be defined as the quantitative analysis of actual economic phenomena based on the concurrent development of theory and observation, related by appropriate methods of inference" [@samuelson1954report]. This accurately represents one the two main approaches to statistical analysis as outlined by @breiman2001statistical, which is known as the data approach. This approach imposes structure onto the available data, it uses the data to approximate a model whose form has been pre-defined by statistical and/or economic theory [@breiman2001statistical]. The alternative to this approach is known as an algorithmic modelling, this doesn not impose any specific structure onto the data, but rather relies on the approximation of the natural form of the data, this structure can map more complex relationships and tends to produce models that predict closer to real values, often at the cost of interpretability [@breiman2001statistical]. Since 2001 the statistical modelling has transitioned to a more algorithmic approach to analysis by embracing the revolution in machine learning that has occured since 2001. @athey2019machine discusses the transition in the economics literature especially, although the transition to algorithmic approaches has been much slower than other fields, there has been an increasing literature on algorithmic models in econometrics. The reason for the slow transition is likely due to differences in culture and a difference in the output of value between the two approaches [@athey2019machine]. These differences cause results from the algorithmic approach to look different from more traditional economic analysis, in turn causing a slower uptake of this approach in modern economics as the output is not immediately comparable or testable against the more traditional approach [@athey2019machine]. We outline some of the differences/challenges below. 

The data approach, also referred to as classical econometrics, is most often used to estimate parameters $$\beta$$ that quantify the relationship between variables $$x$$ and $$y$$, but the algorithmic or machine learning approach does not usually allow for the estimation of these parameters [@mullainathan2017machine]. Rather the focus relies more on the optimal estimation of complex relationships with the parameters of interest being hyperparameters that influence the way that the algorithm is run. Secondly, the complex nature of the non-linear estimation of machine learning models does not lend itself to the econometric notion of statitical significance [@hoepner2021significance]. In models such as the OLS, TWFE or ARIMA models there are clear standard errors and p-values that can easily quantify the confidence level we have that a variable has a significant impact on another. Machine learning most often does not have this type of easily quantifiable number to estimate impact, the complex nature of the algorithm makes it near impossible to fully trace the individual effects of variables on one another [@hoepner2021significance]. Another difference is in the use of statistical tests by the data approach, such as an F-test or Durbin-Watson test, to provide easily calculable measures of the validity of models or other desirable features [@athey2019machine]. Contrary to this, algorithmic methods rely on the specifications chosen and the context of the problem to validate the results [@athey2019machine]. Arguably the biggest difference lies in the econometric aim of causal inference between variables of interest, this is something that machine learning struggles to prove with mainstream methods [@hoepner2021significance]. The classical econometric literature has over time developed methods specifically for causal inference, such as Instrumental Variables and Difference-in-Difference techniques. However, while the literature looking into 'white-box' methods of explainable artificial intelligence has seen growth in recent years, it is not near the level of classical econometrics for causal inference [@hoepner2021significance].

While these differences between the data and algorithmic approach has caused a slower uptake of machine learning techniques in economic literature, these differences also allow for combined techniques that, when used correctly, can make up for weaknesses of the different methods and create a better combined model. There are several examples of techniques that have stemmed from the combination of econometric and machine learning techniques, for example, 
